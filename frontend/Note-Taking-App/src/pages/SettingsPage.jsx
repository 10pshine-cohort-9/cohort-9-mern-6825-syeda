...
const labelClass = "text-[11px] font-mono uppercase tracking-wider text-gray-400";
  ...
  return (
    <button
      ...
      className={`flex sm:flex-col flex-row items-center sm:items-start gap-2 sm:gap-0.5 shrink-0 sm:w-full
        ...
        ${active ? "bg-[#FFF3D0]/60 text-[#10151F]" : "text-gray-500 hover:bg-gray-100 hover:text-gray-800"}`}
    >
      <span className="flex items-center gap-2 sm:gap-2.5">
        <Icon size={15} strokeWidth={2} />
        <span className="text-sm font-medium whitespace-nowrap sm:whitespace-normal">{tab.label}</span>
      </span>
      <span className="hidden sm:block font-mono text-[10.5px] uppercase tracking-wide text-gray-400 pl-[23px]">
        {tab.hint}
      </span>
    </button>
  );
...
const StatCard = ({ icon: Icon, label, value, accent }) => (
  <div
    className="relative flex items-center gap-3 bg-white border border-gray-100 rounded-xl pl-4 pr-4 py-4 overflow-hidden
  ...
  >
    ...
    <div>
      <p className="text-xl font-['Space_Grotesk'] font-bold text-[#10151F] leading-none mb-1">
        {value}
      </p>
...
const LogoutConfirmModal = ({ loading, onConfirm, onCancel }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center px-4">
      ...
      <div
        className="modal-backdrop-in absolute inset-0 bg-black/40 backdrop-blur-[2px]"
      ...
      >
        ...
        <div className="p-3 pt-0 flex items-center gap-2">
          <button
            onClick={onCancel}
            disabled={loading}
            className="flex-1 h-10 text-sm font-semibold text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 disabled:opacity-50 transition-colors duration-150"
          >
            Cancel
          </button>
          <button
    ...
    <div className="flex bg-[#F7F7FA] min-h-screen">
      ...
      <main className="flex-1 px-8 py-6">
        <div className="max-w-3xl mx-auto">
          ...
          <h1 className="font-['Space_Grotesk'] font-bold text-[28px] leading-tight tracking-tight text-[#10151F] mb-1">
            Settings
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-6 sm:gap-8">
            ...
            <nav className="relative flex sm:flex-col flex-row gap-1 sm:w-52 overflow-x-auto sm:overflow-visible -mx-1 px-1 sm:mx-0 sm:px-0">
              <span
                className="hidden sm:block absolute -left-1 w-[3px] h-[56px] rounded-full bg-[#FFC93C] transition-transform duration-200 ease-out motion-reduce:transition-none"
            ...
            <div className="flex-1 min-w-0">
              {activeTab === "profile" && (
                <div key="profile" className="settings-fade-in bg-white border border-gray-100 rounded-xl overflow-hidden">
                  ...
                  <div className="px-6 pb-6">
                    <div className="flex items-end gap-4 -mt-10 mb-6">
                      <div className="relative shrink-0">
                        {profile.avatar ? (
                          ...
                          <div className="w-24 h-24 rounded-full bg-[#FFC93C]/25 text-[#10151F] flex items-center justify-center font-['Space_Grotesk'] font-bold text-2xl ring-4 ring-white shadow-md">
                            {getInitials(user?.name)}
                          </div>
                        )}
                        <button
                          onClick={() => fileInputRef.current?.click()}
                          className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-[#FFC93C] text-[#10151F] flex items-center justify-center border-2 border-white hover:bg-[#f5bf2f] transition-colors duration-150"
                      ...
                      <div className="pb-1">
                        <p className="font-['Space_Grotesk'] font-bold text-lg text-[#10151F] leading-tight">
                          {user?.name || "—"}
                        </p>
                        <p className="text-sm text-gray-500">{user?.email || "—"}</p>
                      </div>
                    </div>
                    {avatarError && <p className="text-xs text-red-500 -mt-4 mb-4">{avatarError}</p>}
                    ...
                    <label className="flex flex-col gap-1.5 mb-6">
                      ...
                      />
                      <span className="self-end text-[11px] font-mono text-gray-400">
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        className="flex items-center gap-1.5 px-4 h-9 text-sm font-semibold text-[#10151F] bg-[#FFC93C] rounded-lg hover:bg-[#f5bf2f] disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-150"
                      >
                ...
                <div key="overview" className="settings-fade-in">
                  <div className="grid sm:grid-cols-3 gap-3 mb-4">
                    <StatCard
                      ...
                      accent={{ bar: "#FFC93C", bg: "rgba(255,201,60,0.18)", icon: "#8a6d1a" }}
                    />
                    <StatCard
                      icon={Pin}
                      label="Pinned notes"
                      value={loading ? "—" : pinnedCount}
                      accent={{ bar: "#E8553D", bg: "rgba(232,85,61,0.1)", icon: "#E8553D" }}
                    />
                    <StatCard
                      icon={Trash2}
                      label="In trash"
                      value={loading ? "—" : trashedNotes.length}
                      accent={{ bar: "#9CA3AF", bg: "#F3F4F6", icon: "#6B7280" }}
                    />
                  </div>
              ...
              {activeTab === "account" && (
                <div key="account" className="settings-fade-in bg-white border border-gray-100 rounded-xl p-6">
                  <div className="flex flex-col gap-4 mb-5">
                    ...
                    <div
                      className="inline-flex w-fit items-center gap-2 px-3 py-1.5 border border-dashed border-gray-300 rounded-md -rotate-1 font-mono text-[11px] uppercase tracking-wider text-gray-500"
                    >
                      <Calendar size={12} />
                  ...
                  <div className="pt-5 border-t border-gray-100">
                    <p className="text-sm font-medium text-gray-800 mb-1">Session</p>
                    ...
                    <button
                      ...
                      disabled={loggingOut}
                      className="flex items-center gap-2 px-4 h-9 text-sm font-medium text-red-600 border border-red-200 rounded-lg hover:bg-red-50 disabled:opacity-50 transition-colors duration-150"
                    >
